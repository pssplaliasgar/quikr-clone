...
async function seedCategories() {
    try {
        ...
        for (const parentData of categoriesData) {
            const existingParent = await queryRunner.query('SELECT id FROM parent_categories WHERE slug = $1', [parentData.slug]);
            let parentId;
            if (existingParent.length > 0) {
                ...
                const parentResult = await queryRunner.query('INSERT INTO parent_categories (name, slug, icon) VALUES ($1, $2, $3) RETURNING id', [parentData.name, parentData.slug, parentData.icon]);
            ...
            for (const subData of parentData.subCategories) {
                const existingSub = await queryRunner.query('SELECT id FROM sub_categories WHERE slug = $1 AND "parentId" = $2', [subData.slug, parentId]);
                let subId;
                if (existingSub.length > 0) {
                    ...
                    const subResult = await queryRunner.query('INSERT INTO sub_categories (name, slug, "parentId") VALUES ($1, $2, $3) RETURNING id', [subData.name, subData.slug, parentId]);
                    subId = subResult[0].id;
                    console.log(`  - Inserted sub-category: ${subData.name}`);
                }
                for (const leafData of subData.leafCategories) {
                    const existingLeaf = await queryRunner.query('SELECT id FROM leaf_categories WHERE slug = $1 AND "subCategoryId" = $2', [leafData.slug, subId]);
                    if (existingLeaf.length === 0) {
                        await queryRunner.query('INSERT INTO leaf_categories (name, slug, "subCategoryId") VALUES ($1, $2, $3)', [leafData.name, leafData.slug, subId]);
